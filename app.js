...
function magicView(){const list=magicData[selected]||[];if(!list.length)return '<div class="empty"><div class="empty-icon">✦</div><b>Sem magias na ficha</b><div class="sub">Não há magias registradas no PDF enviado.</div></div>';const fieldLabels=["Círculo / Escola","Execução","Alcance","Alvo / Área / Efeito","Duração","Resistência"];return '<div class="list powers">'+list.map(m=>{const parts=String(m[1]||"").split(" • ");let circle=parts.shift()||"",cost=parts.pop()||"",fields=parts;let meta='<div class="magic-fields">'+fields.map((v,i)=>'<div class="magic-field"><small>'+esc(fieldLabels[i]||"Informação")+'</small><b>'+esc(v)+'</b></div>').join("")+'<div class="magic-field magic-cost"><small>Custo básico</small><b>'+esc(cost)+'</b></div></div>';return '<details class="power magic"><summary><span class="power-icon">✧</span><b>'+esc(m[0])+'</b><span class="magic-circle">'+esc(circle)+'</span></summary>'+meta+'<div class="power-text">'+esc(m[2])+'</div></details>';}).join('')+'</div><div class="sync">Magias transcritas das fichas enviadas.</div>';}function inventory(c){
