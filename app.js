...
Malekir:{race:"Suraggel (Sulfure)",origin:"Aristocrata",className:"Arcanista",level:4,deity:"Wynna",money:683,hp:26,maxHp:26,mp:31,maxMp:31,def:16,fort:7,ref:8,will:9,init:6,per:6,speed:9,attrs:{FOR:1,DES:4,CON:3,INT:5,SAB:0,CAR:2},
...
Zuri:{race:"Medusa",origin:"Artista",className:"Bardo",level:4,deity:"Wynna",money:767,hp:33,maxHp:33,mp:22,maxMp:22,def:16,fort:5,ref:8,will:4,init:6,per:4,speed:9,attrs:{FOR:1,DES:4,CON:3,INT:2,SAB:0,CAR:4},
...
function migrate(){
  if(state.Neo && state.Neo._sheetVersion!==2){
    const keep={image:state.Neo.image||fixedCharacterImages.Neo,conditions:Array.isArray(state.Neo.conditions)?state.Neo.conditions:[],masterNote:typeof state.Neo.masterNote==="string"?state.Neo.masterNote:"",money:typeof state.Neo.money==="number"?state.Neo.money:951,hp:state.Neo.hp,mp:state.Neo.mp};
    state.Neo=clone(base.Neo); Object.assign(state.Neo,keep); state.Neo._sheetVersion=2; cloudDirty=true;
  }
  if(state.Hippion && state.Hippion._sheetVersion!==2){
    const keep={image:state.Hippion.image||fixedCharacterImages.Hippion,conditions:Array.isArray(state.Hippion.conditions)?state.Hippion.conditions:[],masterNote:typeof state.Hippion.masterNote==="string"?state.Hippion.masterNote:"",money:typeof state.Hippion.money==="number"?state.Hippion.money:678,hp:state.Hippion.hp,mp:state.Hippion.mp};
    state.Hippion=clone(base.Hippion); Object.assign(state.Hippion,keep); state.Hippion._sheetVersion=2; cloudDirty=true;
  }
